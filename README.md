...
Every Account has up/down controls in native and web Settings. Pinned and unpinned Accounts have separate saved orders. `PUT /api/v1/unpinned-order` accepts `{"accountIds":["opaque-account-id"]}` containing every current unpinned Account exactly once and returns the updated Accounts response. The unpinned order survives restarts and credential rotation within the selected database.
