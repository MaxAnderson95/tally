...
Web and mobile Settings have the same **Warm-up** controls and **Menu bar** section. Model lists load automatically when opening Warm-up; preferences are shared with the Mac app. Account cards show failed warm-up attempts, and Settings has a **Resume** button after resolving the cause. Schedule dates use the Mac's timezone and remain visible while quota information is unavailable. Accounts without an applicable window are disabled before selection. Launch at login, database location, listener settings, and quitting Tally are managed on the Mac.
