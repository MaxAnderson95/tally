...
- `packages/core/src/session/projector.ts` copies fork prefixes with original timestamps and sequence numbers and records the parent/before-or-through boundary. Tally excludes timestamps before fork creation, even when ancestors were deleted, and uses a surviving boundary's sequence to exclude same-millisecond prefixes. Child sessions are included. Tally does not fingerprint requests, add V1 remnants, replay events, or add lifetime session totals.
