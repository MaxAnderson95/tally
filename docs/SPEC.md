...
- Explicit refresh retries immediately, bypassing cooldowns, provider Retry-After, and the minimum interval; it joins any in-flight work. Launch/wake retains known cooldowns and a 60-second minimum between attempts. Return scheduling state without waiting for all results.
...
Rates come from the models.dev catalog (`https://models.dev/api.json`). The owner reads it at startup and every six hours, retries after 15 minutes on failure, and keeps the last good catalog in `prices.json` beside the Account store so restarts price activity offline. A changed catalog (by content digest) triggers a rescan; views priced with an older catalog stay visibly stale until then. Until models.dev has been read once, the release-bundled reviewed revision `2026-09-07-r1` prices activity. models.dev lists one rate per component, so its prices are scalar: Anthropic 1-hour cache writes use the listed (5-minute) write rate, Go off-peak/peak rates collapse to the listed rate, and a context tier applies when the prompt exceeds the tier size. A listed `reasoning` rate prices reasoning tokens; otherwise they use the output rate.

Match exact provider/model pairs as models.dev lists them under the same provider ID. Do not strip suffixes, infer a price from recorded cost, or substitute model-family rates. Unlisted models stay visible and unpriced.
...
| Collection | Two-minute cadence; wake with a 60-second minimum; immediate explicit refresh; in-flight joining; automatic 2/4/8/15 backoff and Retry-After; rejected-credential blocking; healthy Account independence; activity refresh during provider cooldown. |
