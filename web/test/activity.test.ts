...
import { apiValue, decodeActivity, tokenLabel, type ActivityData } from '../src/activity.ts'
...
test('Swift activity fixture retains nullable pricing, five components, and 30 calendar buckets', () => {
  ...
  assert.equal(apiValue(data.totals), 'Not priced')
  ...
  assert.equal(tokenLabel(empty), 'no activity')
  assert.equal(apiValue(empty), null)
...
test('Swift pricing fixture exposes partial subset bounds, ranged models, and one revision', () => {
  ...
  assert.equal(apiValue(data.totals), 'at least $0.06')
  ...
  assert.equal(apiValue(data.providers[0].models[0].totals), '$0.05 to $0.06')
  ...
  assert.equal(apiValue(data.trend.days[0].totals), null)
})
