...
Icon colors are cosmetic preferences keyed by a hash of the provider and OpenCode credential row ID within each database namespace. They survive token rotation and app restarts independently of Account identity, readings, pins, and reset commands. Existing saved colors seed these preferences when their credentials are next recognized. Changing the color saves immediately and rolls back on a write failure. A different credential row or database namespace has its own color preference.
