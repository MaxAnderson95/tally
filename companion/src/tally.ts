...
export function createTally(options: unknown = {}) {
      ...
      if (input.action === 'redeem' || input.action === 'redemption' || input.action === 'acknowledge') {
        ...
        try {
          ...
          try {
            ...
            const lookupFault = Fault.safeParse(lookup)
            throw fault('operation_response_unknown', `The ${input.action} response was lost or incompatible; its outcome is uncertain (${input.action}: ${known.success ? known.data.code : 'invalid_response'}, lookup: ${lookupFault.success ? lookupFault.data.code : 'invalid_response'}). Original operation UUID: ${input.operationId}. Read it with redemption using that UUID. Do not resend, generate a replacement UUID, or infer success from changed usage. Ask the user before acknowledgement or a new operation.`)
          }
        }
      }
  ...
  return {
    name: 'tally',
    description: 'Query Tally periodically while working to check remaining subscription usage, freshness, reset times and pacing. status checks the app; accounts lists all Accounts or reads an explicit opaque accountId; activity reads retained provider-level OpenCode activity (today by default); refresh schedules reads without waiting for collection. Queries are model-directed. Prefer status and accounts for routine checks; they read cached state and never contact providers. The app already collects every two minutes, so use refresh only when the user asks for fresh numbers or a reading is stale, and never repeatedly: attempts within 60 seconds of the last one are deferred, and a deferred or cooldown result means wait for nextAttemptAt rather than posting again. Every redeem requires a specific explicit user request to consume one banked reset for the named Account; standing permission, low usage, and keep working do not qualify. Resolve names through accounts and ask the user to clarify ambiguous names to an opaque accountId. Never select an active or fallback Account implicitly. Omit creditId for app selection or supply the specifically requested credit. Nullable applicability means not reported, not zero or ineligible; the provider decides which windows reset. Use one operation UUID per explicitly authorized redemption and retain it. Never automatically retry a mutation or generate a replacement UUID after response loss. The companion attempts one read of that original operation after response loss; if unresolved it reports uncertainty and the original UUID. After acknowledge response loss, ok means the operation was read; null acknowledgedAt means acknowledgement is not confirmed by this read, and acknowledgementRequired shows whether the block still requires acknowledgement. Use redemption with that UUID for further progress reads. Pending is accepted, not confirmed; return promptly and query progress separately. Unknown requires asking the user before acknowledgement or a new operation. Every acknowledge also requires specific explicit user instruction; it releases the block without retrying or changing unknown to confirmed. Tally trusts this local client authorization claim and cannot inspect the conversation. Confirmed outcomes remain confirmed independently of failed usage refresh; do not infer reset effects or success from changed readings. Keep operation IDs, result URLs, acknowledgement requirements and structured faults visible. Null is unknown, not zero. Keep stale readings, partial history and pricing coverage visible. Inventory and activity belong to the Mac running Tally. Activity and API-equivalent estimates are not measured quota consumption or subscription charges.',
    input: Input,
    output: Result,
